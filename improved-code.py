...
class LLMClient:
    ...
    PROVIDER_CONFIGS = {
        ...
        "google": {
            "url": "https://mygemini.gongrzhe.us.kg/api/v1/chat/completions",
            "models_url": "https://mygemini.gongrzhe.us.kg/api/v1/models",
            "default_models": [
                "gemini-1.5-pro",
                "gemini-1.5-flash",
                "gemini-1.0-pro"
    ...
    async def _fetch_ollama_models(self, config: Dict[str, Any]) -> List[str]:
        """Fetch Ollama models with specialized handling."""
        try:
            # Use list endpoint for Ollama
            ...
            # Modern API endpoint
            models_url = f"{ollama_host}/api/models"
            
            session = await self._ensure_session()
            try:
                async with session.get(models_url, timeout=aiohttp.ClientTimeout(total=15)) as response:
                    data = await response.json()
                    logging.debug(f"Ollama API response: {data}")
                ...
                logging.warning(f"Error with modern Ollama API: {e}")
            ...
            try:
                ...
                async with session.get(models_url, timeout=aiohttp.ClientTimeout(total=15)) as response:
                    data = await response.json()
                    logging.debug(f"Ollama tags API response: {data}")
                ...
                logging.warning(f"Error with legacy Ollama API: {e}")
            
            # If all else fails, use defaults
            logging.warning("Unable to fetch Ollama models, using defaults")
            self.available_models["ollama"] = config["default_models"]
            return config["default_models"]
    ...
    async def _call_google(self, messages: List[Dict[str, str]]) -> str:
        ...
        # Implementation similar to other async API calls
    ...
    async def _call_openroute(self, messages: List[Dict[str, str]]) -> str:
        ...
        # Implementation similar to other async API calls
        return "OpenRoute API response"

    async def _call_ollama(self, messages: List[Dict[str, str]]) -> str:
        """Call the Ollama API asynchronously."""
        # Implementation similar to other async API calls
        return "Ollama API response"
...
class ChatSession:
    ...
    async def process_llm_response(self, llm_response: str) -> Tuple[bool, str]:
        ...
        Returns:
            Tuple of (is_tool_call, result_or_response)
            - is_tool_call: True if this was a tool call, False otherwise
            - result_or_response: Tool execution result or original response
        """
        import json
        import re
        ...
        # If we found code blocks that might contain JSON, try each one
        if code_blocks:
            for code_block in code_blocks:
                try:
                    tool_call = json.loads(code_block)
                    if "tool" in tool_call and "arguments" in tool_call:
                        # We found a valid tool call in a code block
                        return await self._process_tool_call(tool_call)
                except json.JSONDecodeError:
                    continue  # Try next code block if this one isn't valid JSON
        
        # If no code blocks or none had valid tool calls, try the whole response
        try:
        ...
        # If we got here, it wasn't a tool call
        return (False, llm_response)
        
    async def _process_tool_call(self, tool_call: Dict[str, Any]) -> Tuple[bool, str]:
        """Process a tool call once it's been identified.
        
        Args:
            tool_call: Parsed tool call dictionary
            
        Returns:
            Tuple of (True, result) to indicate this was a tool call
        """
        ...
        for server in self.servers:
            ...
            if any(tool.name == tool_call["tool"] for tool in tools):
                try:
                    ...
                    # Format as tool result for the LLM to understand
                    return (True, f"Tool execution result for '{tool_call['tool']}': {result_str}")
                    ...
                    try:
                        ...
                        return (True, f"Error executing tool after reconnection: {str(retry_error)}")
    ...
    async def _worker(self, messages: List[Dict[str, str]]) -> None:
        ...
        while True:
            try:
                ...
                logging.info("\nAssistant initial response: %s", llm_response)

                # Process the response and execute any tools
                is_tool_call, result = await self.process_llm_response(llm_response)
                ...
                if is_tool_call:
                    # Don't add the raw JSON tool call to message history
                    # Instead, add a system message with the tool result
                    messages.append({"role": "system", "content": result})
                    
                    # Get a second response from the LLM to interpret the tool result
                    final_response = await self.llm_client.get_response(messages)
                    logging.info("\nAssistant final response: %s", final_response)
                    
                    # Add the final human-friendly response to history
                    messages.append({"role": "assistant", "content": final_response})
                    
                    # Put the final response in the queue for display to user
